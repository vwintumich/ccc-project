...
### NB 09: Learned g Prep (dataset: harder)
*Completed.* Notebook: `notebooks/09_learned_g_prep.ipynb`

**Split:** 240,211 real rows split into train (71,969 / 30%), 
val (48,005 / 20%), test (120,237 / 50%), grouped by 
def_answer_pair_id to prevent leakage. Zero skipped rows — every 
train real row matched to exactly one distractor.

**Critical finding — triplet ordering problem:**
The cosine sanity check in Section 6 revealed a fundamental issue 
with using the harder dataset for triplet training:

- Mean cosine similarity, anchor ↔ positive (true answer): 0.6493 ± 0.1355
- Mean cosine similarity, anchor ↔ negative (distractor): 0.7706 ± 0.0682
- Gap (positive − negative): −0.1213
- Fraction of triplets where positive > negative: 18.1%

Negatives (distractors) are on average MORE similar to anchors 
(definitions) than positives (true answers) are. This is a direct 
consequence of how the harder distractors were constructed: they were 
selected as the top-100 most cosine-similar answer words to each 
definition in stock CALE space. The cosine gap that makes the harder 
dataset challenging for classification (Decision 6, validated in NB05) 
is exactly what makes it problematic for triplet training.

**Implication for triplet loss:** Triplet loss only produces a learning 
signal for "violated" triplets — those where the negative is currently 
closer to the anchor than the positive. With only 18.1% of triplets in 
this configuration, the effective training set is ~13,000 triplets 
rather than 71,969. The other 81.9% contribute zero gradient.

**Decision:** Proceed with Option 1 — run training on the harder 
dataset as-is and evaluate on the validation set. If the learned g 
shows no meaningful difference from stock CALE on the validation ATE, 
this will confirm that distractor circularity is a fundamental blocker 
and motivate Option 2 (online hard negative mining) or Option 3 
(training on easy/random distractors instead). Either outcome is a 
meaningful research finding.

**Connection to open questions:** This finding directly illustrates the 
distractor circularity problem described in PHASE2_CONTEXT.md: 
distractors selected using stock CALE's geometry undermine the training 
signal for fine-tuning that same model. Future experiments should 
explore distractor sets constructed independently of stock CALE 
(e.g., WordNet-structure-based distractors) to break this circularity.

...
Phase 2 experiments write outputs into named subfolders to keep results
from different runs from overwriting each other. The root is:

    data/learned_g/{dataset_name}/

Where `dataset_name` matches the DATASET config variable in
09_learned_g_prep.ipynb (e.g., `harder`, `wordnet`, `easy`).

### Prep Notebook Outputs (CPU, 09_learned_g_prep.ipynb)
...
| `test_stock_clue_emb.npy` | Stock CALE clue-context embeddings for test-set rows. Shape: (N_test, 1024). Sliced from Phase 1 clue_context_embeddings.npy. |

### Training Script Outputs (GPU, scripts/train_g_triplet.py)
...
| File | Description |
|------|-------------|
| `outputs/figures/ate_comparison_{dataset_name}.png` | Bar chart comparing cosine ATE: stock vs. learned g, with confidence intervals. |
| `outputs/figures/neighborhood_{dataset_name}.png` | Neighborhood overlap analysis: how much learned g reorganized the embedding space relative to stock CALE. |

### Phase 2 Data Flow

    data/dataset_{name}.parquet        (from Phase 1 NB05)
    data/embeddings/*.npy              (from Phase 1 NB02)
              │
              ▼
    [09_learned_g_prep.ipynb — CPU]
              │
              ▼
    data/learned_g/{name}/
        split_config.json
        ...
        test_stock_*_emb.npy
              │
              ▼
    [scripts/train_g_triplet.py — GPU, Great Lakes]
              │
              ▼
    data/learned_g/{name}/
        model_{method}_{tag}/
    ...
    [10_learned_g_results.ipynb — CPU]
              │
              ▼
    outputs/figures/ate_comparison_{name}.png
    outputs/figures/neighborhood_{name}.png

...
- Place it in `data/` at the repo root before running notebooks

See `DATA_LICENSE` for full attribution and license details.

`data/id_map.csv` and `data/publisher_lookup.csv` are committed to the repo
and should not be manually edited. `id_map.csv` is generated by
`data_preparation/assign_ids.py` and maps each `clue_id` to a `puzzle_id`.
`publisher_lookup.csv` is the authoritative lookup table for puzzle metadata
extraction. If either file needs to be regenerated, document the reason in
`DECISIONS.md` first.
...
```
ccc-project/
  data/                          # Shared raw and derived data artifacts
                                 # (data.sqlite3, *_raw.csv, *_metadata.csv,
                                 # id_map.csv, publisher_lookup.csv,
                                 # clues_filtered.csv)
  data_preparation/              # Shared upstream notebooks that act on raw
                                 # inputs to produce shared derived artifacts
    00_data_extraction.ipynb     # sqlite -> 6 raw CSVs
    puzzle_metadata.ipynb        # raw -> puzzle_metadata.csv
    structural_filtering.ipynb   # raw -> clues_filtered.csv
    wordplay_metadata.ipynb      # filtered -> wordplay_metadata.csv
    assign_ids.py                # raw -> id_map.csv
    clue_utils.py                # shared definition-finding and <t></t> tagging

  indicator_clustering/          # Unsupervised clustering component
    notebooks/                   # Pipeline notebooks 01–07 (run in order)
      archive/                   # Superseded and exploratory notebooks
    data/                        # Component-internal data files
  ...
  clue_misdirection/             # Supervised learning component
    ...
    data/                        # Component-internal data files (~1.8 GB)
...
`data_preparation/puzzle_metadata.ipynb` depends on `data/id_map.csv` being
present. This file is committed to the repo, so no manual setup step is
required.
...
|-------|----------|-------------|
| 0 | `data_preparation/00_data_extraction` (shared) | Local |

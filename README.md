...
`notebooks/assign_ids.py` and maps each `clue_id` to a `puzzle_id`.
...
`notebooks/puzzle_metadata.ipynb` depends on `data/id_map.csv` being present.
This file is committed to the repo, so no manual setup step is required.

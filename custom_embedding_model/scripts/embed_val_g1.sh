...
# ---------------------------------------------------------------------------
# Generate validation-split embeddings for g1 (the fine-tuned CALE variant
# trained with canonical mean pooling per Decision 20, via NB 09 /
# train_g1.py). Covers all three phrase types: f_clue, f_common_wndef,
# f_common_wnex.
#
# Mean pooling must be used at inference time because that is the extraction
# g1 was trained against. Decision 20 establishes mean pooling as canonical
# for CALE; g1 is the corrected fine-tuned variant.
#
# The script's consistency check is automatically skipped for this run:
# these weights are fine-tuned, so the saved g_stock/f_clue.npy baseline is

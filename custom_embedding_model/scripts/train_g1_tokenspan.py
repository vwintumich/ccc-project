...
HuggingFace `AutoModel` (not SentenceTransformer) so gradients can flow
...
# =============================================================================
# §5 — Concept-aligned embedding extraction
# =============================================================================
#
# Ported verbatim from NB 09 §4.2. The function averages hidden states over
# tokens whose character span lies inside the <t>...</t> delimited region.
# Gradients flow through this selection, which is why we use AutoModel rather
# than SentenceTransformer — SentenceTransformer's pooling layer would apply
# mean-pooling over the whole sequence and break the concept-aligned signal
# that CALE's pretraining objective optimizes.
...
def find_delimiter_char_offsets(text: str) -> tuple:
    ...
    Returns (None, None) if either tag is missing, so the caller can fall
    back to mean pooling defensively.
    """
...
def extract_concept_embedding(model, tokenizer, texts, device, max_length: int = 128):
    ...
    # Allocate in the same dtype as hidden_states so this op composes with
    # autocast/fp16 without unnecessary casts.
    ...
    for i in range(batch_size):
        ...
        if start_char is None:
            ...
            # silently falling back to attention-masked mean pooling avoids
            # crashing a 4-hour training job over one malformed row.
